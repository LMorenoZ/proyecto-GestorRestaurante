  ...
  <div class="container pt-4">
    <div class="card border-0 shadow">
      <div class="row g-0">
        ...
        <div class="col-md-8">
          <div class="card-body">
            <h2 v-if="empleadoInfo">{{ empleadoInfo.nombre }} {{ empleadoInfo.apellido }}</h2>
            <h4 v-if="empleadoInfo">{{ empleadoInfo.puesto }}</h4>
            <span v-if="empleadoInfo" :class="`badge rounded-pill bg-${colorEstado}`">{{ empleadoInfo.logeado ? 'Activo' : 'No activo'}}</span>
            <hr v-if="empleadoInfo">
            <div v-if="empleadoInfo" class="info-section">
              ...
              <div class="info-item">
                <span class="item-title">DUI:</span>
                <span>{{ empleadoInfo.dui }}</span>
              </div>
              <div class="info-item">
                <span class="item-title">Fecha creación:</span>
                <span>{{ fechaFormateada(empleadoInfo.creation.toDate()) }}</span>
              </div>
            </div>
            <button type="button" class="btn btn-outline-primary mt-3" @click="router.push('/administracion/empleados')">Regresar</button>
          </div>
        </div>
      </div>
    </div>
